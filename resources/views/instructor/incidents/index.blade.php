<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800 dark:text-gray-100 tracking-tight">Mis Reportes de Fallas</h2>
                <p class="text-[0.85rem] text-slate-500 dark:text-[#B0B3B8] mt-0.5">Monitorea y gestiona el estado de tus reportes de incidencias.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-[#242526] rounded-[1.25rem] shadow-sm border border-slate-200/60 dark:border-[#3A3B3C] p-4 md:p-5">
                <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-5 w-full">
                    ...
                    <div class="flex flex-col lg:flex-row items-center justify-between gap-4 w-full flex-wrap">
                        <form method="GET" action="{{ route('instructor.incidents.index') }}" class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto flex-grow">
                            ...
                            <div class="relative w-full sm:w-64 lg:w-96 flex-grow">
                                ...
                                @if(request('search'))
                                    <a href="{{ route('instructor.incidents.index', array_filter(request()->except('search'))) }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-[#E11D48] transition-colors">
                            ...
                            <div class="flex items-center gap-2 w-full sm:w-auto max-sm:mt-1 flex-shrink-0">
                                ...
                                @if(request()->anyFilled(['search', 'status']))
                                    <a href="{{ route('instructor.incidents.index') }}" class="flex items-center justify-center px-4 py-2.5 bg-[#FFF1F2] hover:bg-[#FFE4E6] dark:bg-rose-900/20 dark:hover:bg-rose-900/40 text-[#E11D48] dark:text-rose-400 rounded-xl transition-colors font-medium text-[0.85rem] gap-1.5 shadow-sm flex-shrink-0" title="Limpiar filtros">
            ...
            <div class="bg-white dark:bg-[#242526] rounded-[1.25rem] shadow-sm border border-slate-200/60 dark:border-[#3A3B3C] overflow-hidden flex flex-col min-h-[500px]">
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-[#3A3B3C]/50 dark:bg-[#18191A] transition-colors group">
                                    ...
                                    <td class="px-6 py-5 text-right align-top">
                                        <div class="flex items-center justify-end gap-1.5 text-slate-400 dark:text-[#9CA3AF] opacity-80 group-hover:opacity-100 transition-opacity">
                                            <a href="{{ route('instructor.incidents.show', $incident->id) }}" class="p-2 hover:bg-slate-100 dark:bg-[#3A3B3C] hover:text-slate-700 dark:text-gray-200 rounded-lg transition-colors" title="Ver detalle">
                                ...
                                <tr>
                                    <td colspan="4" class="px-8 py-16 text-center text-slate-500 dark:text-[#B0B3B8]">
                                        <div class="flex items-center justify-center flex-col">
                                            ...
                                            <h3 class="text-[0.95rem] font-bold text-slate-700 dark:text-gray-200 mb-1">No tienes reportes de fallas</h3>
                                            @if(request()->anyFilled(['search', 'status']))
                                                <p class="text-[0.85rem] text-slate-500 dark:text-[#9CA3AF] mb-4">No hay reportes que coincidan con los filtros aplicados.</p>
                ...
                <div class="px-6 py-5 border-t border-slate-100 dark:border-[#3A3B3C] flex flex-col sm:flex-row items-center justify-between gap-4 mt-auto style-pagination-none">
                    <div class="text-[0.85rem] text-slate-500 dark:text-[#B0B3B8] font-medium whitespace-nowrap">
                        Mostrando <span class="font-bold text-slate-700 dark:text-gray-200">{{ $incidents->firstItem() ?? 0 }}</span> a <span class="font-bold text-slate-700 dark:text-gray-200">{{ $incidents->lastItem() ?? 0 }}</span> de <span class="font-bold text-slate-700 dark:text-gray-200">{{ $incidents->total() }}</span> reportes
                    </div>
