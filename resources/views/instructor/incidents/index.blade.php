                    ...
                    <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="text-sm text-gray-600 dark:text-gray-400">
                                ...
                                @else
                                    Total de reportes: <strong class="text-gray-900 dark:text-white">{{ $incidents->count() }}</strong>
                                @endif
                            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900/70 dark:to-gray-800/70 backdrop-blur-sm">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Descripción</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Ubicación</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Estado</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Fecha de Reporte</th>
                                <th class="px-6 py-4 text-right text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($incidents as $incident)
                                <tr class="group hover:bg-rose-50/50 dark:hover:bg-gray-700/30 transition-all duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-bold text-gray-900 dark:text-white group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">
                                            {{ $incident->title }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-sm text-gray-600 dark:text-gray-300">{{ $incident->location ?? '—' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            ...
                                            $color = $statusColors[$incident->status] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-500/20 dark:text-gray-300';
                                        @endphp
                                    ...
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            @if(request()->anyFilled(['search', 'status']))
                                                ...
                                                <p class="text-gray-600 dark:text-gray-400 mb-4">No hay reportes que coincidan con los filtros aplicados.</p>
                                                <a href="{{ route('instructor.incidents.index') }}" 
                                            ...
                                            @else
                                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">No tienes reportes registrados</h3>
                                                <p class="text-gray-600 dark:text-gray-400 mb-4">Comienza reportando una nueva falla.</p>
                                                <a href="{{ route('instructor.incidents.create') }}" 
