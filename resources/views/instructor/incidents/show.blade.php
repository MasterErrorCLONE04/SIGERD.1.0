        ...
        <div class="max-w-full mx-auto px-4 sm:px-8 lg:px-12 w-full space-y-6">
            ...
            <div
                ...
                <div class="p-8">
                    ...
                    <div
                        class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6 pb-6 border-b border-gray-200 dark:border-[#3A3B3C] dark:border-gray-700">
                        <div class="flex-1">
                            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-2">
                                {{ $incident->title }}
                            </h1>
                            <div class="flex items-center gap-3 flex-wrap">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold border {{ $color }}">
                                ...
                                <span
                                    class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-400 flex items-center gap-1">
                    ...
                    <div class="mb-6">
                        <h3
                            class="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-3 flex items-center gap-2">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                        ...
                        <div
                            ...
                            <p class="text-gray-900 dark:text-gray-100 dark:text-white font-medium">#{{ $incident->id }}
                            </p>
                        </div>
                    </div>
                    ...
                    @if ($incident->initial_evidence_images && count($incident->initial_evidence_images) > 0)
                        <div class="mb-6">
                            <h3
                                class="text-lg font-bold text-gray-900 dark:text-gray-100 dark:text-white mb-4 flex items-center gap-2">
                                <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                @foreach ($incident->initial_evidence_images as $imagePath)
                                    <div class="group relative overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105 bg-gray-100 dark:bg-gray-700 cursor-pointer w-64 shrink-0"
                                        ...
                                        <div
                                            ...
                                            <svg class="w-10 h-10 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg"
                                        ...
                                        <div
                                            class="absolute bottom-3 right-3 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                                            <span
                                                class="text-xs font-medium text-gray-700 dark:text-gray-200 dark:text-gray-300">Click
                                                para ampliar</span>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    @if ($incident->status === 'resuelto')
                        <div
                            ...
                            <div class="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    <h3
                                        class="text-xl font-bold text-green-900 dark:text-green-100 flex items-center gap-2">
                                        ✅ Incidente Resuelto
                                    </h3>
                                    <p class="text-sm text-green-700 dark:text-green-300">El problema ha sido solucionado
                                        exitosamente</p>
                                </div>
                            </div>
                            ...
                            <div class="mb-4 bg-white/50 dark:bg-gray-800/30 rounded-lg p-4">
                                ...
                                <p class="text-gray-900 dark:text-gray-100 dark:text-white font-medium">
                                    ...
                                    <span
                                        class="text-sm text-gray-600 dark:text-gray-300 dark:text-gray-400">({{ $incident->resolved_at->diffForHumans() }})</span>
                                </p>
                            </div>
                            ...
                            @if ($incident->final_evidence_images && count($incident->final_evidence_images) > 0)
                                <div class="bg-white/50 dark:bg-gray-800/30 rounded-lg p-4">
                                    <h4
                                        class="text-sm font-semibold text-green-700 dark:text-green-300 mb-3 flex items-center gap-2">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                    ...
                                    <div class="flex gap-4 overflow-x-auto pb-4 custom-scrollbar mt-3">
                                        @foreach ($incident->final_evidence_images as $imagePath)
                                            <div class="group relative overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105 bg-gray-100 dark:bg-gray-700 cursor-pointer w-64 shrink-0"
                                                onclick="openImageModal('{{ asset('storage/' . $imagePath) }}')">
                                                <img src="{{ asset('storage/' . $imagePath) }}" alt="Evidencia Final"
                                                    class="w-64 h-48 object-cover shrink-0">
                                                <div
                                                    class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
                                                    <svg class="w-10 h-10 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg"
                                                ...
                                                {{-- Badge de completado --}}
                                                <div
                                                    class="absolute top-3 left-3 bg-green-500/90 backdrop-blur-sm px-2.5 py-1 rounded-lg">
                    ...
                    @if ($incident->status === 'asignado')
                        <div
                            class="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-xl p-5">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-blue-900 dark:text-blue-100 font-semibold mb-1">🔄 Trabajo en Progreso
                                    </p>
                                    <p class="text-blue-700 dark:text-blue-300 text-sm">Este reporte ya ha sido asignado a
                                        un trabajador. Te notificaremos cuando esté resuelto y podrás ver las evidencias del
                                        trabajo realizado.</p>
                                </div>
                            </div>
                        </div>
                    ...
                    @if ($incident->status === 'pendiente de revisión')
                        <div
                            class="mb-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-xl p-5">
                            <div class="flex items-start gap-3">
                                <div
                                ...
                                <div>
                                    <p class="text-yellow-900 dark:text-yellow-100 font-semibold mb-1">⏳ Pendiente de
                                        Revisión</p>
                                    <p class="text-yellow-700 dark:text-yellow-300 text-sm">Este reporte está siendo
                                        revisado por el equipo administrativo. Pronto será convertido en una tarea y
                                        asignado a un trabajador.</p>
                                </div>
                            </div>
                        </div>
                    @endif
    ...
    <div id="imageModal" class="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 hidden items-center justify-center p-4"
        ...
        <div class="relative max-w-7xl max-h-full" onclick="event.stopPropagation()">
            ...
            <button onclick="closeImageModal()"
                class="absolute top-4 right-4 text-white bg-black/50 hover:bg-black/70 rounded-full p-3 transition-all hover:scale-110">
