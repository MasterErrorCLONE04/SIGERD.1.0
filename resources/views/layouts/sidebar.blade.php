        ...
        <div class="px-6 py-6 border-b border-slate-100 dark:border-gray-700 flex-shrink-0">
            <div class="flex items-center gap-4" :class="sidebarCollapsed ? 'justify-center w-full ml-[-8px]' : ''">
                <div class="relative flex-shrink-0">
                    ...
                    @endif
                    <div
                        class="absolute bottom-0 right-0 w-3.5 h-3.5 bg-[#10B981] border-2 border-white dark:border-gray-800 rounded-full">
                    </div>
                </div>
                <div x-show="!sidebarCollapsed" x-transition class="flex-1 min-w-0">
                    <p class="text-[0.95rem] font-bold text-[#1E293B] dark:text-white truncate">{{ Auth::user()->name }}
                    </p>
                    <div class="mt-0.5">
                        <span
                            class="inline-flex items-center px-2 py-0.5 text-[0.65rem] font-bold rounded-full bg-[#EFF6FF] dark:bg-blue-900/30 text-[#3B82F6] dark:text-blue-400 capitalize tracking-wide">
        ...
        <div class="px-4 py-6 border-t border-slate-100 dark:border-gray-700 flex-shrink-0 space-y-2">
            ...
            <form method="POST" action="{{ route('logout') }}">
                ...
                <button type="submit" :title="sidebarCollapsed ? 'Cerrar Sesión' : ''"
                    class="w-full flex items-center px-4 py-2.5 rounded-lg text-[#EF4444] dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 font-medium transition-colors"
                    :class="sidebarCollapsed ? 'justify-center' : ''">
                    <svg class="w-[1.2rem] h-[1.2rem] flex-shrink-0 text-[#EF4444] dark:text-red-400"
