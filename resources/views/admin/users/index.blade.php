    ...
    <div class="p-6 lg:p-8 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="bg-white rounded-[1.25rem] shadow-sm border border-slate-200/60 p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-500 mt-1">
                                @if(request('search'))
                                    Se encontraron <strong class="text-slate-800">{{ $users->count() }}</strong> resultado(s) para "{{ request('search') }}"
                                @else
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <button onclick="openModal('createUserModal')" class="flex items-center gap-2 bg-[#1A202C] hover:bg-[#2D3748] text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-colors shadow-sm focus:ring-2 focus:ring-slate-200 whitespace-nowrap">
            ...
            <div class="bg-white rounded-[1.25rem] shadow-sm border border-slate-200/60 overflow-hidden flex flex-col min-h-[500px]">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-100">
                                <th class="px-8 py-5 text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                ...
                                <th class="px-8 py-5 text-[11px] font-bold text-slate-500 uppercase tracking-widest text-right">
                                    Acciones
                                </th>
                            </tr>
                        </thead>
                            ...
                            @forelse ($users as $user)
                                <tr class="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div class="relative flex-shrink-0">
                                                @if($user->hasProfilePhoto())
                                                    <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}" class="w-12 h-12 rounded-[0.8rem] object-cover bg-slate-100 ring-1 ring-slate-200/50">
                                                @else
                                                    <div class="w-12 h-12 rounded-[0.8rem] bg-[#E8E8E8] text-slate-600 flex items-center justify-center font-bold text-sm ring-1 ring-slate-200/50">
                                                        {{ $user->initials }}
                                                    </div>
                                                @endif
                                                <div class="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-[#10B981] border-2 border-white rounded-full"></div>
                                            </div>
                                            <div>
                                    ...
                                    <td class="px-8 py-5">
                                        ...
                                        <div class="flex items-center gap-1.5 mt-1 text-[0.8rem] text-[#10B981] font-medium">
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex items-center justify-end gap-2.5">
                                            <a href="{{ route('admin.users.show', $user->id) }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200/80 text-slate-600 hover:text-slate-800 text-[0.8rem] font-semibold rounded-lg transition-colors">
                                            ...
                                            <a href="{{ route('admin.users.edit', $user->id) }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-[#F0F5FF] hover:bg-[#E0EBFF] text-[#2563EB] text-[0.8rem] font-semibold rounded-lg transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                Editar
                                            </a>
                                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este usuario?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2 bg-[#FEF2F2] hover:bg-[#FEE2E2] text-[#DC2626] text-[0.8rem] font-semibold rounded-lg transition-colors border-none cursor-pointer">
                                ...
                                <tr>
                                    <td colspan="4" class="px-8 py-16 text-center text-slate-500">
                                        <div class="flex items-center justify-center flex-col">
                                            <svg class="w-12 h-12 text-slate-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z"></path></svg>
                ...
                <div class="px-8 py-5 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 mt-auto">
                    <div class="text-[0.85rem] text-slate-500 font-medium">
                        Mostrando <span class="font-bold text-slate-700">{{ $users->firstItem() ?? 0 }}</span> a <span class="font-bold text-slate-700">{{ $users->lastItem() ?? 0 }}</span> de <span class="font-bold text-slate-700">{{ $users->total() }}</span> resultados
                    </div>
                    
                    <div class="flex items-center gap-1.5">
                        {{-- Botón Anterior --}}
                        @if ($users->onFirstPage())
                            <span class="w-9 h-9 flex items-center justify-center rounded-lg border border-slate-200 text-slate-300 cursor-not-allowed">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"></path></svg>
                            </span>
                        @else
                            <a href="{{ $users->previousPageUrl() }}" class="w-9 h-9 flex items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-colors shadow-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"></path></svg>
                            </a>
                        @endif
                        ...
                        @if ($users->hasMorePages())
                            <a href="{{ $users->nextPageUrl() }}" class="w-9 h-9 flex items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-colors shadow-sm">
