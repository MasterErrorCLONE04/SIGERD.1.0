            ...
            <div class="bg-white dark:bg-[#242526] rounded-[1.25rem] shadow-sm border border-slate-200/60 dark:border-[#3A3B3C] p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <h2 class="text-[1.35rem] font-bold text-slate-800 dark:text-gray-100 tracking-tight">Registro de Incidentes</h2>
                    ...
                    <div class="flex flex-col xl:flex-row items-center gap-4 w-full md:w-auto">
                        <form method="GET" action="{{ route('admin.incidents.index') }}" class="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                            <div class="relative flex-grow sm:w-80">
                                ...
                                @if(request('search'))
                                    <a href="{{ route('admin.incidents.index', array_filter(request()->except('search'))) }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 dark:text-[#9CA3AF] hover:text-slate-600 dark:text-gray-300">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    </a>
                                @endif
                            </div>
                            <div class="flex gap-2 w-full sm:w-auto">
                                <input type="date" name="created_at_from" value="{{ request('created_at_from') }}" class="w-full sm:w-auto px-4 py-2.5 bg-white dark:bg-[#242526] border border-slate-200/80 rounded-xl text-sm text-slate-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-slate-100 focus:border-slate-300 transition-colors flex-grow">
                        ...
                        <div class="flex items-center gap-2 w-full xl:w-auto">
                            @if(request()->anyFilled(['search', 'created_at_from']))
                                <a href="{{ route('admin.incidents.index') }}" class="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-slate-100 dark:bg-[#3A3B3C] hover:bg-slate-200 text-slate-700 dark:text-gray-200 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors whitespace-nowrap">
