                ...
                <form method="GET" action="{{ route('worker.tasks.index') }}" class="space-y-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                        ...
                        <div class="text-sm text-gray-600 dark:text-gray-400">
                            @if(request()->anyFilled(['search', 'status', 'priority']))
                                Se encontraron <strong class="text-gray-900 dark:text-white">{{ $tasks->count() }}</strong> resultado(s)
                            @else
                                Total de tareas: <strong class="text-gray-900 dark:text-white">{{ $tasks->count() }}</strong>
                            @endif
                        </div>
                    </div>
                </form>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900/70 dark:to-gray-800/70 backdrop-blur-sm">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Prioridad</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Estado</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Fecha Límite</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Asignado por</th>
                                ...
                                <tr class="group hover:bg-blue-50/50 dark:hover:bg-gray-700/30 transition-all duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-600 dark:text-gray-300">
                                            @if($task->deadline_at < now() && !in_array($task->status, ['finalizada', 'cancelada', 'realizada']))
                                                <span class="text-red-600 dark:text-red-400 font-medium">{{ $task->deadline_at->format('d/m/Y') }}</span>
                                            ...
                                            @elseif($task->deadline_at <= now()->addDays(7) && !in_array($task->status, ['finalizada', 'cancelada', 'realizada']))
                                                <span class="text-yellow-600 dark:text-yellow-400 font-medium">{{ $task->deadline_at->format('d/m/Y') }}</span>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-sm text-gray-600 dark:text-gray-300">{{ $task->createdBy->name ?? 'N/A' }}</span>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            @else
                                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">No tienes tareas asignadas</h3>
                                                <p class="text-gray-600 dark:text-gray-400">Las tareas que te asignen aparecerán aquí.</p>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
