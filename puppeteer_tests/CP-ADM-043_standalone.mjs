...
(async () => {
    ...
    try {
        ...
        // FILL FORM WITHOUT PHOTOS
        await page.type('#createIncidentModal input[name="title"]', 'Test Sin Fotos');
        await page.type('#createIncidentModal textarea[name="description"]', 'Prueba de validación sin fotos.');
        await page.type('#createIncidentModal input[name="location"]', 'Oficina Central');

        // Remove required attribute from file input to allow form submission for testing server-side validation
        await page.evaluate(() => {
            document.querySelector('#createIncidentModal input[name="initial_evidence_images[]"]').required = false;
        });
        ...
        const success = await page.evaluate(() => document.body.innerText.includes('Debe subir al menos una imagen de evidencia.'));
